...
const PLACEHOLDERS = {
  ...
  history: "ابحث في سير الأعلام (مثلاً: معركة بدر)...",
  scholars: "ابحث عن عالم (مثلاً: ابن تيمية)...",
  jarh: "ابحث في الجرح والتعديل (مثلاً: يحيى بن معين)...",
...
export default function SearchApp() {
  ...
  const formatResults = (hits, type, queryTerm) => {
    const highlightText = (text) => text; // simple fallback
    return hits.map(hit => {
      ...
      if (type === 'hadith') {
        ...
        snippet = truncateText(answerPreview, 220);
        ...
        snippet = (hit.trj || hit.summary || "").substring(0, 220) + "...";
            ...
            <div className="results-list">
              {results.map((item, i) => (
                <div key={i} className="result-item" style={{ borderBottom: '1px solid #e2e8f0', padding: '16px 0' }}>
                  ...
                  <h3 className="result-title text-xl font-bold font-serif mb-2 text-blue-800 hover:underline">
